...
Cod Lambda (Node.js 20, CommonJS). Modulele de bază au **zero dependențe externe**
— doar builtin-uri: `http/https`, `zlib`, `stream`, `dns`, `url`, `crypto`,
`node:test`. Excepția e `lambda-entry.js` (wire-ul de deploy), care are nevoie de
`ioredis` + `mongodb` + `@aws-sdk/client-secrets-manager` bundled în zip-ul Lambda.
Nu intră în bundle-ul vite; se va deploya ca Lambda + Function URL (decizia §8.6
din spec).
...
| `rate-limit.js` | C7 — rate-limit anti-abuz, obligatoriu la lansare (§0, §8.4). Limite default: **3 audite/zi/IP** (`feed_audit:rl:ip:{yyyy-mm-dd}:{sha256(ip)}`, TTL 48h — IP hash-uit, zero IP-uri brute în store), **1 job concurent/domeniu** (`feed_audit:rl:lock:domain:{host}`, TTL 1h, release compare-and-match ca să nu eliberăm lock-ul altui job), **cap global 500/zi** (`feed_audit:rl:global:{yyyy-mm-dd}`, TTL 48h, frâna de cost compute). La depășirea limitei/IP se marchează `feed_audit:rl:ts:{sha256(ip)}` (TTL 7 zile); cu `TURNSTILE_SECRET` configurat și token `x-turnstile-token` lipsă → 429 cu `turnstile_required: true` (verificarea reală a tokenului = la deploy). Store injectabil: `memoryStore()` (local/test) / `redisStore({client})` (thin wrapper ioredis-like, **Valkey — NU Upstash**; `delIfMatch` via Lua compare-and-delete). Wrappere de degradare: `guardedStore({store})` transformă orice eroare a store-ului în `StoreUnavailableError` (503) și `unavailableStore()` refuză orice operație (503) — ambele **fail-closed**: fără fallback silențios pe memorie (un cold start Lambda ar reseta contorul → limita ocolibilă, bugul fixat 2026-09-11). Fără store → `noopLimiter()` (rate-limit dezactivat explicit — doar local/test, niciodată la deploy). **Endpointul NU intră în nicio allow-list** (§8.1) — catyai.io nu stă în spatele caty-shop-waf; rate-limit-ul la aplicație e singura apărare. |
| `lambda-entry.js` | **Entry point-ul Lambda** (`feed-audit/lambda-entry.handler`) — wire-ul de deploy, anterior doar în `/tmp/feed-audit-deploy` (pierdut la reboot). Face: (1) **limiter → Valkey AWS** prin secretul dedicat `caty/production/ELASTICACHE_REDIS_URL` (aceeași instanță ca `src/services/redis.js` din caty-api; ioredis cu `lazyConnect` + `commandTimeout` 10s + `retryStrategy`, listener `error`); indisponibil/lipsă → `unavailableStore()` = POST 503 „retry later", GET status funcțional — **fail-closed, nu fallback per-container**. (2) **Mongo best-effort**: `MONGODB_URI` din secretul JSON `caty/production` → telemetrie `feed_audits` + comparator read-only `retail_product_offers` (contract fabrici: doar citiri); eșecul Mongo nu afectează limiterul. Inițializare memoizată per execution environment. Env-urile `ELASTICACHE_REDIS_URL`/`MONGODB_URI` suprascriu Secrets Manager (debug). Deploy: zip cu `feed-audit/` la rădăcină + `node_modules` (inclusiv `ioredis`, `mongodb`, `@aws-sdk/client-secrets-manager` — runtime-ul nodejs20.x NU include niciun AWS SDK). |
...
- **Deploy-ul Lambda**: `lambda-entry.js` e entry point-ul (`feed-audit/lambda-entry.handler`)
  și face wire-ul real: Valkey (rate-limit, fail-closed), Mongo (`feed_audits`,
  `retail_product_offers` read-only), Turnstile (`TURNSTILE_SECRET`), webhook
  `NOTIFY_WEBHOOK_URL`. Handlerul de bază `index.js` rămâne fără store →
  `noopLimiter()` — doar pentru local/test, NU de expus public așa (§0: fără
  rate-limit endpointul nu se lansează).
